...
const searchTextBox = document.querySelector("#search-bar input");
const searchButton = document.querySelector("#search-bar a");
const recipeCardsSection = document.querySelector("#recipes");

// Displays random recipe on page load.
displaySearchResults([recipes[Math.floor(Math.random() * (recipes.length))]]);

// When users search something.
searchTextBox.addEventListener("keydown", event => {if (event.key === "Enter") searchRecipes()});
searchButton.addEventListener("click", searchRecipes);

function searchRecipes() {
    // Filter through the recipes with user's query to search.
    // User's query.
    const query = searchTextBox.value;
    // Search by filtering through recipes.
    // Combine the contents of the attributes of object and determine if the query matched any of it.
    const filteredRecipes = recipes.filter(recipe => Object.values(recipe).reduce((info, attributeContents) => info + attributeContents, "").toLowerCase().includes(query.toLowerCase()));
    // Sort recipes by difficulty.
    const sortedResults = filteredRecipes.sort(sortByRating);
    // Clear recipeCards on page and display recipe search results.
...
function displaySearchResults(recipeResults) {
    recipeResults.forEach(recipe => {
        // Each recipe card is made up of an image and contents. The contents are made up of tags, a name, rating, and description.
        // The image.
        const image = `<img src="${recipe.image}">`;
        // The contents.
        // The tags.
        const tags =    '<span class="recipe-tags">' +
                        recipe.tags.reduce((htmlButtons, tag) => {
                            return htmlButtons + `<button>${tag}</button>`;
                        }, "") +
                        `</span>`;
        // The name.
        const name = `<h2>${recipe.name}</h2>`;
        // The rating.
        const rating =  `<span class="recipe-rating" role="img" aria-label="Rating: ${recipe.rating} out of 5 stars.">` +
                        Array.from({length: Math.floor(recipe.rating)}, () => ``).reduce(stars => stars + `<span aria-hidden="true" class="icon-star">⭐</span>`, ``) +
                        Array.from({length: 5 - Math.floor(recipe.rating)}, () => ``).reduce(stars => stars + `<span aria-hidden="true" class="icon-star-empty">☆</span>`, ``) +
                        `</span>`;
        const description = `<p class="description" aria-hidden="true">${recipe.description}</p>`;
        // The contents.
        const contents =    `<div class="recipe-content">` +
                            tags +
                            name +
                            rating +
                            description +
